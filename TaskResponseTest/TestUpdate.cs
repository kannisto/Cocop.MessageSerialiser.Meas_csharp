...
namespace TaskResponseTest
{
    ...
    {
        [TestMethod]
        public void TaskUpdateResp_Read()
        {
            var filepath = TestCommon.TestHelper.TestFileFolder + @"\TaskUpdateResponse.xml";
        ...
        [TestMethod]
        public void TaskUpdateResp_Read_NoExtension()
        {
            var filepath = TestCommon.TestHelper.TestFileFolder + @"\TaskUpdateResponse_NoExtension.xml";
            var testObject = new TaskResponse(ReadFile(filepath));
        ...
        [TestMethod]
        public void TaskUpdateResp_Create()
        {
            var statusReport = new TaskStatusReport(taskId: "anothertask", procId: "anotherproc")
            {
                RequestStatus = TaskingRequestStatusCodeType.Rejected
            };

            var testObject = TaskResponse.CreateUpdateResponse(statusReport);
            ...
            TaskUpdateResp_Create_Assert(testObjectIn);
        }

        private void TaskUpdateResp_Create_Assert(TaskResponse testObjectIn)
        {
            ...
            Assert.AreEqual(TaskingRequestStatusCodeType.Rejected, testObjectIn.StatusReports[0].RequestStatus);
        }
        ...
        private void Validate(byte[] xmlBytes)
        {
            ...
            {
                validator.Validate(xmlStream, typeof(XsdNs.UpdateResponseType));
            }
        }
    }
}
